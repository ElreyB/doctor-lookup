export function lineCreator(doctors) {
  doctors.forEach(function(doctor) {
    $("#doctor-results").append(
      `<li class="well col-md-8 col-md-offset-2">
      <div class="row">
        <div class="col-md-5 doctor-info">
          <img class="doctor-image" src=${checkImage(
            doctor.profile.image_url
          )} alt="Doctor's photo">
          <a href="${websiteFitter(doctor.practices[0].website)}">
          <h4 class="doctor-name">Dr. ${doctor.profile.first_name} ${doctor
        .profile.last_name},${doctor.profile.title}</h4>
            </a>
        </div>
        <div class="col-md-7 doctor-info">
            <p>${doctor.practices[0].name}</p>
            <p>${doctor.practices[0].visit_address.street}</p> <p>${doctor
        .practices[0].visit_address.city},${doctor.practices[0].visit_address
        .state} ${doctor.practices[0].visit_address.zip}</p>
...
export function createSpecialtyCheckboxes(specialties) {
  specialties.forEach(specialty => {
    $("#specialties-list").append(
      `<input type="checkbox" name="specialty-list" value=${specialty.uid}> ${specialty.name}.<br>`
    );
  });
}
